...
Always inspect the delegate's diff before telling the user the work is done — a delegate's "success" report is a claim, not evidence. Revert only delegate-created mistakes when you are sure they are not user changes. If the delegate leaves the repo in a worse state or changes unrelated files, stop and report the issue with the diff summary.
