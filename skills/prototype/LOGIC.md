...
Because it's one file with nothing to install, you can hand it to a non-developer such as a designer, a PM, or a domain expert, and let them feel the model for themselves. So it speaks their language, not the code's.
...
- **A class or module with a clear set of methods** when the logic genuinely owns ongoing internal state.
...
Once the prototype has answered its question, capture the answer, then capture the prototype the way the [SKILL](SKILL.md) describes. The logic-specific mapping: the validated reducer / machine / function set lifts into the real module (the decision, absorbed); the HTML shell rides along to the throwaway branch that keeps the prototype as a primary source. Being one self-contained file, it stays trivially re-runnable there.
